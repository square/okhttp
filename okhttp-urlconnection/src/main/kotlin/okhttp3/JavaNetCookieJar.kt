...
class JavaNetCookieJar(private val cookieHandler: CookieHandler) : CookieJar {
  ...
  /**
   * Convert a request header to OkHttp's cookies via [HttpCookie]. That extra step handles multiple
   * cookies in a single request header, which [Cookie.parse] doesn't support.
   */
  private fun decodeHeaderAsJavaNetCookies(url: HttpUrl, header: String): List<Cookie> {
    ...
    while (pos < limit) {
      ...
      result.add(Cookie.Builder().name(name).value(value).domain(url.host).build())
      pos = pairEnd + 1
    }
    return result
  }
