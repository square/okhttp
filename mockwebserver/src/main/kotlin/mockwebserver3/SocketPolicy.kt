 ...
 *
 * Be careful when using values like [DISCONNECT_AT_END], [SHUTDOWN_INPUT_AT_END] and
 * [SHUTDOWN_OUTPUT_AT_END] that close a socket after a response, and where there are follow-up
 * requests. The client is unblocked and free to continue as soon as it has received the entire
 * response body. If and when the client makes a subsequent request using a pooled socket the server
 * may not have had time to close the socket. The socket will be closed at an indeterminate point
 * before or during the second request. It may be closed after client has started sending the
...
enum class SocketPolicy {
   ...
   * immediately after the response and will close the connection when the client's socket is
   * exhausted.
   *
  ...
  /**
   * Transmit a `HTTP/1.1 100 Continue` response before reading the HTTP request body. Typically
   * this response is sent when a client makes a request with the header `Expect: 100-continue`.
   */
  EXPECT_CONTINUE,

  /**
   * Transmit a `HTTP/1.1 100 Continue` response before reading the HTTP request body even if the
   * client does not send the header `Expect: 100-continue` in its request.
   */
  CONTINUE_ALWAYS
}
