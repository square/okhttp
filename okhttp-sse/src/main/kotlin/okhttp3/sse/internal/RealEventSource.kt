...
class RealEventSource(private val request: Request, private val listener: EventSourceListener) :
    EventSource, ServerSentEventReader.Callback, Callback {
  private lateinit var call: RealCall

  fun connect(client: OkHttpClient) {
    val client = client.newBuilder().eventListener(EventListener.NONE).build()
  ...
  fun processResponse(response: Response) {
    response.use {
      ...
      if (!body.isEventStream()) {
        listener.onFailure(
            this, IllegalStateException("Invalid content-type: ${body.contentType()}"), response)
        return
      }
      ...
      val response = response.newBuilder().body(EMPTY_RESPONSE).build()
