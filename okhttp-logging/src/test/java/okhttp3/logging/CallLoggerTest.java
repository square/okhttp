...
import okhttp3.logging.CallLogger.Level;
...
public final class CallLoggerTest {
  ...
  @Test public void isPlaintext() throws IOException {
    assertTrue(CallLogger.isPlaintext(new Buffer()));
    assertTrue(CallLogger.isPlaintext(new Buffer().writeUtf8("abc")));
    assertTrue(CallLogger.isPlaintext(new Buffer().writeUtf8("new\r\nlines")));
    assertTrue(CallLogger.isPlaintext(new Buffer().writeUtf8("white\t space")));
    assertTrue(CallLogger.isPlaintext(new Buffer().writeByte(0x80)));
    assertFalse(CallLogger.isPlaintext(new Buffer().writeByte(0x00)));
    assertFalse(CallLogger.isPlaintext(new Buffer().writeByte(0xc0)));
  }
