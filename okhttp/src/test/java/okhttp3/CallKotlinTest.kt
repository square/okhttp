...
@Timeout(30)
class CallKotlinTest(val server: MockWebServer) {
  ...
  fun testHeadAfterPut() {
    ...
    request = Request.Builder().url(endpointUrl).head().build()
    // 204
    client.newCall(request).execute()
  ...
  fun staleConnectionNotReusedForNonIdempotentRequest() {
    ...
    server.enqueue(MockResponse().setBody("a").setSocketPolicy(SocketPolicy.SHUTDOWN_OUTPUT_AT_END))
    server.enqueue(MockResponse().setBody("b"))

    val requestA = Request.Builder().url(server.url("/")).build()
  ...
  @Test
  fun exceptionsAreReturnedAsSuppressed() {
