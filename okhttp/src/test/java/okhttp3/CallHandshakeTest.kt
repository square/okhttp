...
class CallHandshakeTest {
  ...
  @RegisterExtension @JvmField var platform = PlatformRule()
  ...
  fun testDefaultHandshakeCipherSuiteOrderingTls12Restricted() {
    ...
    assertThat(handshakeEnabledCipherSuites)
        .containsExactlyElementsOf(expectedConnectionCipherSuites(client))
  }
  ...
  fun advertisedOrderInRestricted() {
    assertThat(ConnectionSpec.RESTRICTED_TLS.cipherSuites)
        .containsExactly(
            TLS_AES_128_GCM_SHA256,
            TLS_AES_256_GCM_SHA384,
  ...
  private fun expectedConnectionCipherSuites(client: OkHttpClient): Set<String> {
    ...
    //    return client.connectionSpecs.first().cipherSuites!!.map { it.javaName
    // }.intersect(defaultEnabledCipherSuites)
