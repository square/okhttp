...
class CertificatePinnerKotlinTest {
  ...
  fun successfulFindMatchingPins() {
    ...
    val expectedPins = listOf(Pin("first.com", certA1Sha256Pin), Pin("first.com", certB1Sha256Pin))
  ...
  fun successfulFindMatchingPinsForWildcardAndDirectCertificates() {
    ...
    val expectedPins =
        listOf(Pin("*.example.com", certA1Sha256Pin), Pin("a.example.com", certB1Sha256Pin))
  ...
  fun wildcardHostnameShouldNotMatchThroughDot() {
    val certificatePinner =
        CertificatePinner.Builder().add("*.example.com", certA1Sha256Pin).build()
  ...
  fun successfulFindMatchingPinPunycode() {
    val certificatePinner = CertificatePinner.Builder().add("σkhttp.com", certA1Sha256Pin).build()
  ...
  companion object {
    internal var certA1: HeldCertificate = HeldCertificate.Builder().serialNumber(100L).build()
    internal var certA1Sha256Pin = CertificatePinner.pin(certA1.certificate)

    private var certB1 = HeldCertificate.Builder().serialNumber(200L).build()
    internal var certB1Sha256Pin = CertificatePinner.pin(certB1.certificate)

    private var certC1 = HeldCertificate.Builder().serialNumber(300L).build()
