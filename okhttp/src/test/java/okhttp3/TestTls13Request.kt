...
class TestTls13Request {
  ...
  @Test
  fun testQuiche() {
    val client = buildClient(TLS_13)
    try {
        sendRequest(client, "https://quic.tech:4433/")
    } finally {
  ...
  companion object {
    ...
    private val TLS13_CIPHER_SUITES =
        arrayOf(CipherSuite.TLS_AES_128_GCM_SHA256, CipherSuite.TLS_AES_256_GCM_SHA384,
    ...
    private val TLS_12 =
        ConnectionSpec.Builder(ConnectionSpec.RESTRICTED_TLS).tlsVersions(TlsVersion.TLS_1_2)
            .build()
    ...
    @JvmStatic fun main(args: Array<String>) {
      ...
      val urls = asList("https://enabled.tls13.com", "https://www.howsmyssl.com/a/check",
    ...
    private fun buildClient(vararg specs: ConnectionSpec): OkHttpClient {
      return OkHttpClient.Builder().connectionSpecs(specs.toList()).build()
    }
    ...
    private fun sendRequest(client: OkHttpClient, url: String) {
      ...
      client.newCall(request).execute().use { response ->
        ...
        println(handshake!!.tlsVersion.toString()
            + " "
