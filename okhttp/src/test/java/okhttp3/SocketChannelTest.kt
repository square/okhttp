...
@Timeout(6)
@Tag("slow")
class SocketChannelTest(val server: MockWebServer) {
  ...
  private val handshakeCertificates = run {
    ...
    val heldCertificate =
        HeldCertificate.Builder().commonName(hostname).addSubjectAlternativeName(hostname).build()
  ...
  fun testConnection(socketMode: SocketMode) {
    ...
    val url =
        if (socketMode is TlsInstance) "https://$hostname:${server.port}/get"
        else "http://$hostname:${server.port}/get"
