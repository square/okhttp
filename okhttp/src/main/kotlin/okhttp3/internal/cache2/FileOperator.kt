...
/**
 * Read and write a target file. Unlike Okio's built-in `Okio.source(java.io.File file)` and
 * `Okio.sink(java.io.File file)` this class offers:
 *
 * * **Read/write:** read and write using the same operator.
 * * **Random access:** access any position within the file.
 * * **Shared channels:** read and write a file channel that's shared between multiple operators.
 * Note that although the underlying [FileChannel] may be shared, each [FileOperator] should not be.
 */
internal class FileOperator(private val fileChannel: FileChannel) {
  ...
  /**
   * Copy [byteCount] bytes from the file at [pos] into `sink`. It is the caller's responsibility to
   * make sure there are sufficient bytes to read: if there aren't this method throws an
   * `EOFException`.
   */
