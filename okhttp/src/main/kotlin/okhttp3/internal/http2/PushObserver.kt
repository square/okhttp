 ...
 *
 * While [onReset] may occur at any time, the following callbacks are expected in order, correlated
 * by stream ID.
 *
 * * [onRequest]
 ...
 *
 * Return true to request cancellation of a pushed stream. Note that this does not guarantee future
 * frames won't arrive on the stream ID.
 */
interface PushObserver {
  ...
  /**
   * The response headers corresponding to a pushed request. When [last] is true, there are no data
   * frames to follow.
   *
   * @param streamId server-initiated stream ID: an even number.
