 ...
 *
 * This is used directly for providers where class name is known e.g. the Google Play Provider but
 * we can't compile directly against it, or in fact reliably know if it is registered and on
 * classpath.
 */
open class AndroidSocketAdapter(private val sslSocketClass: Class<in SSLSocket>) : SocketAdapter {
  ...
  private val setAlpnProtocols = sslSocketClass.getMethod("setAlpnProtocols", ByteArray::class.java)
  ...
  companion object {
    ...
    /**
     * Builds a SocketAdapter from an observed implementation class, by grabbing the Class reference
     * to perform reflection on at runtime.
     *
