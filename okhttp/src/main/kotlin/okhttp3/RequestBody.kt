...
abstract class RequestBody {
   ...
   * for this nonstandard interaction will use it. As of 2019-01, the only widely-used
   * implementation of this pattern is [gRPC] [grpc].
   *
   * Because the encoding of interleaved data is not well-defined for HTTP/1, duplex request bodies
   * may only be used with HTTP/2. Calls to HTTP/1 servers will fail before the HTTP request is
   * transmitted. If you cannot ensure that your client and server both support HTTP/2, do not use
   * this feature.
   *
   * ### Duplex APIs
   *
   ...
   * reads of the [ResponseBody]. For duplex request bodies, [writeTo] should return quickly,
  ...
  /**
   * Returns true if this body expects at most one call to [writeTo] and can be transmitted at most
   * once. This is typically used when writing the request body is destructive and it is not
   * possible to recreate the request body after it has been sent.
   *
   * This method returns false unless it is overridden by a subclass.
   *
   * By default OkHttp will attempt to retransmit request bodies when the original request fails due
   * to any of:
   *
