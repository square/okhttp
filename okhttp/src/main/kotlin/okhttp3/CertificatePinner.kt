 ...
 * authorities either known or unknown to the application's user. This class currently pins a
 * certificate's Subject Public Key Info as described on [Adam Langley's Weblog] [langley]. Pins are
 * either base64 SHA-256 hashes as in [HTTP Public Key Pinning (HPKP)] [rfc_7469] or SHA-1 base64
 * hashes as in Chromium's [static certificates] [static_certificates].
 *
 ...
 * ```
 *
 * Follow up by pasting the public key hashes from the exception into the certificate pinner's
 * configuration:
 *
 * ```
 * CertificatePinner certificatePinner = new CertificatePinner.Builder()
 ...
 * `www.publicobject.com` you must configure both hostnames. Or you may use patterns to match sets
 * of related domain names. The following forms are permitted:
 *
 ...
 * ```
 * Note that any other form is unsupported. You may not use asterisks in any position other than the
 * leftmost label.
 *
 ...
 *
 * [charles]: http://charlesproxy.com [fiddler]: http://fiddlertool.com [langley]:
 * http://goo.gl/AIx3e5 [owasp]: https://www.owasp.org/index.php/Certificate_and_Public_Key_Pinning
 * [rfc_7469]: http://tools.ietf.org/html/rfc7469 [static_certificates]: http://goo.gl/XDh6je
 */
...
internal constructor(
  ...
  fun check(hostname: String, peerCertificates: List<Certificate>) {
    return check(hostname) {
      (certificateChainCleaner?.clean(peerCertificates, hostname) ?: peerCertificates).map {
