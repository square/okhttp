...
import okhttp3.internal.http.InterceptorCall
...
class RealCall(
    ...
    override fun enqueue(responseCallback: Callback) {
        if (isOnDispatcherThread()) {
            InterceptorCall(this).enqueue(responseCallback)
        } else {
    ...
    private fun isOnDispatcherThread(): Boolean = onCallThread.get()
        ...
        fun executeOn(executorService: ExecutorService) {
            ...
            try {
                executorService.execute {
                    onCallThread.set(true)
                    try {
                        this.run()
                    } finally {
                        onCallThread.set(false)
                    }
                }
                success = true
    ...
    companion object {
        val onCallThread = object : ThreadLocal<Boolean>() {
