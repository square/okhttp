...
class CallDecoratorTest {
  ...
  fun testSecureRequest() {
    ...
    val client: OkHttpClient =
      clientTestRule.newClientBuilder().enableTls().addInterceptor(AlwaysHttps).build()
  ...
  fun testInsecureRequestChangedToSecure() {
    ...
    val request = Request.Builder().url(
        server.url("/").newBuilder().scheme("http").build(),
      ).build()
  ...
  fun testWrappedCallIsObserved() {
    ...
    // TODO reimplement this test for the non Decorator world

    val client: OkHttpClient = clientTestRule.newClientBuilder().addInterceptor({ chain ->
...
//            }
//            assertThat(chain.request().tag<String>()).isEqualTo("wrapped")
        }
      }).addInterceptor({ chain ->
        // Wrap here
        val updatedRequest = chain.request().newBuilder().tag<String>("wrapped").build()
...
//          }
  ...
  fun testCanShortCircuit() {
    ...
    val client: OkHttpClient = clientTestRule.newClientBuilder().build()

    val redirectingClient: OkHttpClient =
      clientTestRule.newClientBuilder().addInterceptor(object : ClientForkingInterceptor<Unit>() {
          override fun clientKey(request: Request) = Unit

          override fun OkHttpClient.Builder.buildForKey(key: Unit): OkHttpClient {
            // fully override
            return client
          }
        }).addInterceptor {
...
private object AlwaysHttps : Interceptor {
  override fun intercept(chain: Interceptor.Chain): Response {
    val request = chain.request()
    val updatedRequest = if (request.url.scheme == "http") {
