...
internal object CertificateAdapters {
  ...
  private val algorithmParameters: DerAdapter<Any?> =
      Adapters.usingTypeHint { typeHint ->
        when (typeHint) {
          // This type is pretty strange. The spec says that for certain algorithms we must encode
          // null
  ...
  internal val generalName: DerAdapter<Pair<DerAdapter<*>, Any?>> =
      Adapters.choice(generalNameDnsName, generalNameIpAddress, Adapters.ANY_VALUE)
   ...
   * ```
   */
  internal val name: DerAdapter<Pair<DerAdapter<*>, Any?>> = Adapters.choice(rdnSequence)
  ...
  internal val certificate: BasicDerAdapter<Certificate> =
      Adapters.sequence(
          ...
          algorithmIdentifier,
          Adapters.BIT_STRING,
          decompose = { listOf(it.tbsCertificate, it.signatureAlgorithm, it.signatureValue) },
          construct = {
            Certificate(
  ...
  internal val privateKeyInfo: BasicDerAdapter<PrivateKeyInfo> =
      Adapters.sequence(
          ...
          decompose = { listOf(it.version, it.algorithmIdentifier, it.privateKey) },
          construct = {
            PrivateKeyInfo(
                version = it[0] as Long,
